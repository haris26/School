...
namespace DataSeed
{
    class CharlieSeed
    {
        ...
        static string sourceData = @"C:\Projects\school\Charlie1.xls";
        ...
        static void getAssets()
        {
            ...
            foreach (DataRow row in rawData.Rows)
            {
                string catName = row.ItemArray.GetValue(2).ToString();
                 string userName = row.ItemArray.GetValue(6).ToString();
                ...
                Asset asset = new Asset()
                {
                    ...
                    Vendor = row.ItemArray.GetValue(1).ToString(),
                    // Type = (AssetType)Enum.Parse(typeof(AssetType), row.ItemArray.GetValue(2).ToString()),
                    Model = row.ItemArray.GetValue(3).ToString(),
                    SerialNumber = row.ItemArray.GetValue(4).ToString(),
                    // Description = row.ItemArray.GetValue(3).ToString(),

                    // Price = Convert.ToDouble(row.ItemArray.GetValue(5).ToString()),
                    Status = (AssetStatus)Enum.Parse(typeof(AssetStatus), getString(row, 5)),
                      User = user
                };
                N++;
